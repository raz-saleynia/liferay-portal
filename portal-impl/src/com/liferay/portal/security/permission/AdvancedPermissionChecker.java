	...
	protected boolean isCompanyAdminImpl(long companyId) throws Exception {
		...
		try {
			if (RoleLocalServiceUtil.hasUserRole(
					user.getUserId(), user.getCompanyId(), RoleConstants.ADMINISTRATOR,
					true)) {
				return true;
			}
			String endplayAdminoRole = GetterUtil.getString(PropsUtil.get("endplay.administrator.role"), "EndPlay Administrator");
			Role role = RoleLocalServiceUtil.loadGetRole(user.getCompanyId(), endplayAdminoRole);
			return UserLocalServiceUtil.hasRoleUser(role.getRoleId(), user.getUserId());
		} catch (Exception e) {
			_log.error(e);
		}
		return false;
