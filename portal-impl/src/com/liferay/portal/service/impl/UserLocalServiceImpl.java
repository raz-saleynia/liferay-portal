...
import com.endplay.portlet.custom.RoleLimitsException;
import com.endplay.portlet.custom.UserLimitsException;
import com.endplay.portlet.journal.model.PortalProperties;
import com.endplay.portlet.journal.service.ConfigurationPropertyLocalServiceUtil;
import com.endplay.portlet.journal.service.PortalPropertiesLocalServiceUtil;
...
public class UserLocalServiceImpl extends UserLocalServiceBaseImpl {
	...
	public static final String SEND_FORGOT_PASSWORD_MAIL = "SEND_FORGOT_PASSWORD_MAIL";
	...
	public final static String MAIL_FORMAT = "mail_format";
	public final static String SEND_MAIL_BY_SENDGRID="SendGrid";

	public final static String SENDGRID_UTIL_CLASS= "com.endplay.feeds.util.SendGridUtil";
	public final static String SENDGRID_UTIL_METHOD= "sendMailBySendGrid";
	...
	public void addRoleUsers(long roleId, long[] userIds)
			throws PortalException, SystemException {
		// validate limit when assigning users to regular role
		// userIds: active user ids
		Role role = rolePersistence.fetchByPrimaryKey(roleId);
		List<User> allUsers = userLocalService.getRoleUsers(roleId);
		long[] totalActiveUserIds = new long[allUsers.size()];
		int totalCount = 0;
		for (int i = 0; i < allUsers.size(); i++) {
			if (allUsers.get(i).getStatus() == WorkflowConstants.STATUS_APPROVED) {
				totalActiveUserIds[i] = allUsers.get(i).getUserId();
				totalCount ++;
			}
		}
		for (long userId : userIds) {
			if (!ArrayUtils.contains(totalActiveUserIds, userId)) {
				totalCount ++;
			}
		}
		int limit = getUpperLimitByRole(role);
		if (limit > 0 && limit < totalCount) {
			throw new RoleLimitsException(role.getName());
		}
	...
	private int getUpperLimitByRole(Role role) throws SystemException {

		long companyId = role.getCompanyId();
		StringBuffer roleLimitPropertyName = new StringBuffer("Role.");
		roleLimitPropertyName.append(StringUtils.replace(role.getName(), " ", "."));
		List<PortalProperties> roleLimitPortalProperties = PortalPropertiesLocalServiceUtil
				.findByPropertyName(roleLimitPropertyName.toString(), companyId);
		if (CollectionUtils.isEmpty(roleLimitPortalProperties)) {
			return -1;
		}
		return GetterUtil.getInteger(roleLimitPortalProperties.get(0).getPropertyValue());
	}
	...
	public User addUserWithWorkflow(
		...
		validateUserLimits(companyId, UserLimitsException.ADD);
	...
	public void sendPassword(
		...
		try {
			sendMail = ConfigurationPropertyLocalServiceUtil.getString(companyId, company.getGroup().getGroupId(), SEND_FORGOT_PASSWORD_MAIL);
		} catch (Exception e) {
			_log.error("Get" +  SEND_FORGOT_PASSWORD_MAIL + "DDL Error!");
		}

		if (sendMail != null && sendMail.equalsIgnoreCase(SEND_MAIL_BY_SENDGRID)){
			try {
				Map<String, String> emailInfo = new HashMap<String, String>();

				emailInfo.put(TO, toAddress);
				emailInfo.put(TO_NAME, toName);
				subject = subject.replace("[$PORTAL_URL$]",  serviceContext.getPortalURL());
				emailInfo.put(SUBJECT, subject);
				body = replaceBody(body, passwordResetURL, serviceContext.getRemoteAddr(), serviceContext.getRemoteHost(),
						serviceContext.getUserAgent(), newPassword, user.getUserId(), user.getScreenName(), toName, serviceContext.getPortalURL(),
						fromName, fromAddress);
				emailInfo.put(HTML, body);
				emailInfo.put(FROM, fromAddress);
				emailInfo.put(FROM_NAME, fromName);
				emailInfo.put(MAIL_FORMAT, HTML);

				Class<?> sendMailClass = Class.forName(SENDGRID_UTIL_CLASS);
				Object sendMailObject = sendMailClass.newInstance();
				Method method = sendMailClass.getMethod(SENDGRID_UTIL_METHOD, Long.class, Map.class);
				method.invoke(sendMailObject, new Object[]{company.getGroup().getGroupId(), emailInfo});
			} catch (Exception e) {
				sendMailByDefault(body, companyId, passwordResetURL, serviceContext, user,
						newPassword, fromAddress, fromName, subject, toAddress, toName);
			}
		} else {
			sendMailByDefault(body, companyId, passwordResetURL, serviceContext, user,
					newPassword, fromAddress, fromName, subject, toAddress, toName);
	...
	private String replaceBody(String body, String passwordResetURL, String remoteAddr, String remoteHost,
							   String userAgent, String newPassword, long userId, String screenName, String toName, String portalUrl,
							   String fromName, String fromAddress) {

		body = body.replace("[$PASSWORD_RESET_URL$]", passwordResetURL != null ? passwordResetURL : org.apache.commons.lang.StringUtils.EMPTY);
		body = body.replace("[$REMOTE_ADDRESS$]",  remoteAddr != null ? remoteAddr : org.apache.commons.lang.StringUtils.EMPTY);
		body = body.replace("[$REMOTE_HOST$]", remoteHost != null ? remoteHost : org.apache.commons.lang.StringUtils.EMPTY);
		body = body.replace("[$USER_AGENT$]", userAgent != null ? userAgent : org.apache.commons.lang.StringUtils.EMPTY);
		body = body.replace("[$USER_ID$]", String.valueOf(userId));
		body = body.replace("[$USER_PASSWORD$]", newPassword != null ? newPassword : org.apache.commons.lang.StringUtils.EMPTY);
		body = body.replace("[$USER_SCREENNAME$]", screenName != null ? screenName : org.apache.commons.lang.StringUtils.EMPTY);
		body = body.replace("[$TO_NAME$]", toName != null ? toName : org.apache.commons.lang.StringUtils.EMPTY);
		body = body.replace("[$PORTAL_URL$]", portalUrl != null ? portalUrl : org.apache.commons.lang.StringUtils.EMPTY);
		body = body.replace("[$FROM_NAME$]", fromName != null ? fromName : org.apache.commons.lang.StringUtils.EMPTY);
		body = body.replace("[$FROM_ADDRESS$]", fromAddress != null ? fromAddress : org.apache.commons.lang.StringUtils.EMPTY);

		return body;
	}

	private void sendMailByDefault(String body, long companyId, String passwordResetURL, ServiceContext serviceContext, User user,
								   String newPassword, String fromAddress, String fromName, String subject, String toAddress, String toName) {
		SubscriptionSender subscriptionSender = new SubscriptionSender();
	...
	public User updateStatus(long userId, int status)
		...
		if (status == 0) {
			validateUserLimits(user.getCompanyId(), UserLimitsException.UPDATE);

			// validate user roles

			List<UserGroupRole> userGroupRoles = userGroupRolePersistence.findByUserId(userId);

			List<Role> roles = userPersistence.getRoles(userId);
			long[] roleIds = null;
			...
			validateRoleLimit(userGroupRoles, roleIds, user, user.getCompanyId(), RoleLimitsException.UPDATE_STATUS);
		}
	...
	public User updateUser(
		...
		// validate roles limit

		validateRoleLimit(userGroupRoles, roleIds, user, company.getCompanyId(), RoleLimitsException.ADD);
		// Roles
	...
	private void validateUserLimits(long companyId, int type) throws SystemException, PortalException {
		List<PortalProperties> portalPropertiesArr = PortalPropertiesLocalServiceUtil.findByPropertyName("User.Upper.Limit", companyId);
		int userUpperLimit = 0;
		if (portalPropertiesArr != null && portalPropertiesArr.size() > 0) {
			userUpperLimit = GetterUtil.getInteger(portalPropertiesArr.get(0).getPropertyValue());
		}
		if (userUpperLimit > 0) {
			int userTotalCount = searchCount(companyId, null, null, null, null, null, 0, null, true);
			if (userUpperLimit <= userTotalCount) {
				throw new UserLimitsException(type);
			}
		}
	}

	private void validateRoleLimit(List<UserGroupRole> userGroupRoles, long[] roleIds, User user, long companyId, int type) throws SystemException, PortalException {

		List<PortalProperties> portalPropertiesArr = PortalPropertiesLocalServiceUtil.findByCompanyId(companyId);
		List<PortalProperties> portalPropertiesRoleArr = new ArrayList<PortalProperties>();

		// filter potal properties of role-upper-lmit
		for (PortalProperties portalProperties : portalPropertiesArr) {
			if (portalProperties.getPropertyName().startsWith("Role.")) {
				portalPropertiesRoleArr.add(portalProperties);
			}
		}

		for (PortalProperties portalProperties : portalPropertiesRoleArr) {
			int roleUpperLimit = GetterUtil.getInteger(portalProperties.getPropertyValue());
			if (roleUpperLimit <= 0) {
				continue;
			}
			String roleName = StringUtils.replace(StringUtils.replace(portalProperties.getPropertyName(), "Role.", ""), ".", " ");
			Role role = rolePersistence.fetchByC_N(companyId, roleName);
			if (role == null) {
				continue;
			}
			// validate Organization Roles, Regular Roles and site roles
			if (role.getType() == RoleConstants.TYPE_ORGANIZATION || role.getType() == RoleConstants.TYPE_SITE) {
				validateUserGroupRoleLimit(userGroupRoles, role, roleUpperLimit, companyId, type);
			} else if (role.getType() == RoleConstants.TYPE_REGULAR) {
				validateRegularRoleLimit(roleIds, user, role, roleUpperLimit, type);
			}
		}
	}

	private void validateUserGroupRoleLimit(List<UserGroupRole> userGroupRoles, Role role, int roleUpperLimit, long companyId, int type) throws SystemException, PortalException {

		if (CollectionUtils.isEmpty(userGroupRoles) || role == null || roleUpperLimit <= 0) {
			return;
		}

		for (UserGroupRole userGroupRole : userGroupRoles) {
			if (role.getRoleId() == userGroupRole.getRoleId()) {
				List<UserGroupRole> userGroupRoleByRoleIds = userGroupRolePersistence.findByRoleId(userGroupRole.getRoleId());
				List<UserGroupRole> activeUserGroupRoleByR_Cs = new ArrayList<UserGroupRole>();

				if (userGroupRoleByRoleIds == null) {
					continue;
				}

				// filter out users which are not belong to current company or inactve
				for (UserGroupRole userGroupRoleByRoleId : userGroupRoleByRoleIds) {
					if (userGroupRoleByRoleId.getGroup().getCompanyId() == companyId && userGroupRoleByRoleId.getUser().getStatus() == 0) {
						activeUserGroupRoleByR_Cs.add(userGroupRoleByRoleId);
					}
				}

				// ignore roles which alreay assigned to current user
				if (activeUserGroupRoleByR_Cs.contains(userGroupRole)) {
					continue;
				}

				if (roleUpperLimit <= activeUserGroupRoleByR_Cs.size()) {
					throw new RoleLimitsException(role.getName(), type);
				}
			}
		}
	}

	private void validateRegularRoleLimit(long[] roleIds, User user, Role role, int roleUpperLimit, int type) throws SystemException, PortalException {

		if (ArrayUtils.isEmpty(roleIds) || user == null || role == null || roleUpperLimit <= 0) {
			return;
		}

		for (long roleId : roleIds) {
			if (role.getRoleId() == roleId) {
				List<User> usersByRoleId = rolePersistence.getUsers(roleId);
				List<Long> activeUserIdsByRoleId = new ArrayList<Long>();

				if (usersByRoleId == null) {
					continue;
				}

				// filter out inactve users
				for (User userByRoleId : usersByRoleId) {
					if (userByRoleId.getStatus() == 0) {
						activeUserIdsByRoleId.add(userByRoleId.getUserId());
					}
				}

				// ignore roles which alreay assigned to current user
				if (activeUserIdsByRoleId.contains(user.getUserId())) {
					continue;
				}

				if (roleUpperLimit <= activeUserIdsByRoleId.size()) {
					throw new RoleLimitsException(role.getName(), type);
				}
			}
		}
