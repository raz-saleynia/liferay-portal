...
import com.endplay.portal.xml.SAXParserObjectFactory;
import com.endplay.portal.xml.SAXParserPool;
...
public class SAXReaderImpl implements SAXReader {
	protected static SAXParserPool parserPool = new SAXParserPool(new SAXParserObjectFactory());
	...
	public Document read(File file, boolean validate)
		...
		try {
			...
			try {
				saxParser = (SAXParser)parserPool.borrowObject();
			}
			catch (Exception ex) {
				_log.error(ex);
				
				saxParser = new SAXParser(new XIncludeAwareParserConfiguration());
			}
			...
			if (saxParser != null) {
				try {
					parserPool.returnObject(saxParser);
				}
				catch (Exception ex) {
					_log.error(ex);
				}
	...
	protected org.dom4j.io.SAXReader getSAXReader(SAXParser saxParser, boolean validate) {
