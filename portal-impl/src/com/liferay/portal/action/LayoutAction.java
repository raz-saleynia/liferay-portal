...
public class LayoutAction extends Action {
	...
	protected ActionForward processLayout(
		...
		try {
			...
			if (themeDisplay.isLifecycleResource()) {
				...
				boolean isSharedRefPage = GetterUtil.getBoolean(request.getAttribute("isSharedRefPage"), false);
				if (isSharedRefPage) {
					long originPlid = GetterUtil.getLong(request.getAttribute("targetSharePagePlid"), 0L);
					if (originPlid != 0L) {
						layout.setPlid(originPlid);
	...
	protected Portlet processPortletRequest(
		...
		if (lifecycle.equals(PortletRequest.ACTION_PHASE)) {
			...
			try {
				...
				boolean isSharedRefPageUpdate = GetterUtil.getBoolean(request.getAttribute("isSharedRefPageUpdate"), false);
				if (isSharedRefPageUpdate) {
					boolean isSharedRefPage = GetterUtil.getBoolean(request.getAttribute("isSharedRefPage"), false);
					if (isSharedRefPage) {
						long sourcePlid = GetterUtil.getLong(request.getAttribute("sourceSharePagePlid"), 0L);
						if (sourcePlid != 0L) {
							layout.setPlid(sourcePlid);
							themeDisplay.setLayout(layout);
							request.setAttribute(WebKeys.THEME_DISPLAY, themeDisplay);
						}
					}
				...
				if (isSharedRefPageUpdate) {
					boolean isSharedRefPage = GetterUtil.getBoolean(request.getAttribute("isSharedRefPage"), false);
					if (isSharedRefPage) {
						long targetPlid = GetterUtil.getLong(request.getAttribute("targetSharePagePlid"), 0L);
						if (targetPlid != 0L) {
							layout.setPlid(targetPlid);
							themeDisplay.setLayout(layout);
							request.setAttribute(WebKeys.THEME_DISPLAY, themeDisplay);
						}
					}
	...
	protected Event serializeEvent(
		...
		value = (Serializable)SerializableUtil.deserialize(serializedValue);
