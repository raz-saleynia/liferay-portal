...
import com.endplay.portal.util.ReorderLayoutsUtil;
import com.endplay.portlet.articledistribution.model.Distribution;
import com.endplay.portlet.articledistribution.service.DistributionLocalServiceUtil;
import com.endplay.portlet.journal.JournalArticleDataConstants;
import com.endplay.portlet.journal.model.ExtLayout;
import com.endplay.portlet.journal.service.ConfigurationPropertyLocalServiceUtil;
import com.endplay.portlet.journal.service.ExtLayoutLocalServiceUtil;
import com.endplay.portlet.journal.service.LayoutBackupLocalServiceUtil;
import com.endplay.portlet.layout.LayoutDataConstants;
...
public class StagingImpl implements Staging {
	...
	public void copyRemoteLayouts(
		...
		if (layoutIdMap == null) {
			...
			Iterator<Entry<Long, Boolean>> itr1 =
	...
	public void publishLayouts(
		...
		//**********************************************
		//  reference page: Create local module portletPreferences in target site
		//**********************************************
		String[] rowIds = addOrUpdatePortletPreferences(parameterMap, privateLayout);
		...
		//**********************************************
		//  EXT: BACK LAYOUT SETTINGS WHILE PUBLISHING
		//**********************************************
		boolean enableResetLayoutChanges = false;
		try {
			Group group = GroupLocalServiceUtil.getGroup(sourceGroupId);
			enableResetLayoutChanges = GetterUtil.getBoolean(ConfigurationPropertyLocalServiceUtil.getString(group.getCompanyId(), sourceGroupId, LayoutDataConstants.ENABLE_RESET_LAYOUT_CHANGES), false);
		} catch (Exception e) {
			_log.error(String.format("Error getting configuration property [%s].", LayoutDataConstants.ENABLE_RESET_LAYOUT_CHANGES, e));
		}

		if (enableResetLayoutChanges) {
			LayoutBackupLocalServiceUtil.backupLayoutsSettings(sourceGroupId, targetGroupId, privateLayout, layoutIds, endDate);
		}
		//**********************************************
		//  EXT: BACK LAYOUT SETTINGS WHILE PUBLISHING
		//**********************************************
		Boolean enableReorderLiveLayouts = GetterUtil.getBoolean(ConfigurationPropertyLocalServiceUtil.getBoolean(GroupLocalServiceUtil.getGroup(sourceGroupId).getCompanyId(), sourceGroupId, LayoutDataConstants.ENABLE_REORDER_LIVE_LAYOUTS));
		if (Boolean.TRUE.equals(enableReorderLiveLayouts)) {
			try {
				ReorderLayoutsUtil.sitePagesReorder(sourceGroupId, 0L, 0);
			} catch (Exception e) {
				_log.error("Reorder failed, while publish page to live." + e);
			}
		}

		//**********************************************
		// EXT: EXT_LAYOUT PUBLISH
		//**********************************************
		if (rowIds != null) {
			for (String plidstr : rowIds) {
				long plid = Long.valueOf(plidstr);
				Distribution distribution = DistributionLocalServiceUtil.findDistributionByTargetPlid(plid);
				if (distribution != null) {
					if (distribution.getPublishType() == JournalArticleDataConstants.DistributionType.COPY.getValue() && distribution.getPublishStatus() != JournalArticleDataConstants.Status.PUBLISHED_SHARE.getValue()) {
						distribution.setPublishStatus(JournalArticleDataConstants.Status.PUBLISHED_SHARE.getValue());
						DistributionLocalServiceUtil.updateDistribution(distribution);
					} else if (distribution.getPublishType() == JournalArticleDataConstants.DistributionType.REFERENCE.getValue()) {
						ExtLayout stageExtLayout = null;
						try {
							stageExtLayout = ExtLayoutLocalServiceUtil.getExtLayout(plid);
						} catch (PortalException e) {
							//do nothing
						} catch (SystemException e) {
							_log.error("Error when get ext layout!", e);
						}

						if (stageExtLayout != null) {
							try {
								String uuid = stageExtLayout.getUUID();
								String sourceUUID = stageExtLayout.getSourceUUID();
								long sourceStageGroupId = stageExtLayout.getSourceGroupId();
								long targetStageGroupId = stageExtLayout.getGroupId();

								Group sourceGroup = GroupLocalServiceUtil.getGroup(sourceStageGroupId);
								Group targetGroup = GroupLocalServiceUtil.getGroup(targetStageGroupId);

								Layout sourceLiveLayout = LayoutLocalServiceUtil.getLayoutByUuidAndGroupId(sourceUUID, sourceGroup.getLiveGroupId(), privateLayout);
								Layout sourceStageLayout = LayoutLocalServiceUtil.getLayoutByUuidAndGroupId(sourceUUID, sourceStageGroupId, privateLayout);
								Layout targetLiveLayout = LayoutLocalServiceUtil.getLayoutByUuidAndGroupId(uuid, targetGroup.getLiveGroupId(), privateLayout);

								//TODO publish localize module to live. Then save/update extlayout to live
								ExtLayout extLiveLayout = ExtLayoutLocalServiceUtil.getExtLayoutByUUIDAndGroupId(uuid, targetGroup.getLiveGroupId());
								if (extLiveLayout == null) {
									ExtLayout liveExtLayout = (ExtLayout) stageExtLayout.clone();
									//update value to live
									liveExtLayout.setPlid(targetLiveLayout.getPlid());
									liveExtLayout.setGroupId(targetLiveLayout.getGroupId());

									liveExtLayout.setSourcePlid(sourceLiveLayout.getPlid());
									liveExtLayout.setSourceGroupId(sourceLiveLayout.getGroupId());
									liveExtLayout.setCreateDate(new Date());
									ExtLayoutLocalServiceUtil.addExtLayout(liveExtLayout);
									if (distribution != null) {
										distribution.setPublishStatus(JournalArticleDataConstants.Status.PUBLISHED_SHARE.getValue());
										DistributionLocalServiceUtil.updateDistribution(distribution);
									}
									//**********************************************
									// Add portletReferences for local module
									//**********************************************
									String extTypeSettings = stageExtLayout.getTypesettings();
									Properties p = new Properties();
									p.load(new StringReader(extTypeSettings));
									String localModulePortletIds = p.getProperty("local-module");
									if (StringUtils.isNotEmpty(localModulePortletIds)) {
										String[] columnPortlets = localModulePortletIds.split(StringPool.COMMA);
										for (int i = 0; i < columnPortlets.length; i++) {
											String portletId = columnPortlets[i].trim();
											List<PortletPreferences> stagePortletPreferences = PortletPreferencesLocalServiceUtil.getPortletPreferences(sourceStageLayout.getPlid(), portletId);
											if (stagePortletPreferences.size() > 0) {
												PortletPreferences portletPreferences = stagePortletPreferences.get(0);
												String preferences = portletPreferences.getPreferences();
												if (StringUtils.isNotEmpty(preferences)) {
													preferences = preferences.replaceAll(String.valueOf(targetStageGroupId), String.valueOf(targetGroup.getLiveGroupId()));
												}
												Portlet portlet = PortletLocalServiceUtil.getPortletById(portletId);
												PortletPreferencesLocalServiceUtil.addPortletPreferences(
														targetGroup.getCompanyId(), 0, 3, sourceLiveLayout.getPlid(), portletId, portlet,
														preferences);
											}
										}
									}
								} else {
									extLiveLayout.setTitle(stageExtLayout.getTitle());
									extLiveLayout.setName(stageExtLayout.getName());
									extLiveLayout.setDescription(stageExtLayout.getDescription());
									extLiveLayout.setLocalOverride(stageExtLayout.getLocalOverride());
									extLiveLayout.setKeywords(stageExtLayout.getKeywords());
									extLiveLayout.setRobots(stageExtLayout.getRobots());
									extLiveLayout.setTypesettings(stageExtLayout.getTypesettings());
									extLiveLayout.setModifiedDate(new Date());
									ExtLayoutLocalServiceUtil.updateExtLayout(extLiveLayout);
									//**********************************************
									// Update portletReferences for local module
									//**********************************************
									String extTypeSettings = extLiveLayout.getTypesettings();
									Properties p = new Properties();
									p.load(new StringReader(extTypeSettings));
									String localModulePortletIds = p.getProperty("local-module");
									if (StringUtils.isNotEmpty(localModulePortletIds)) {
										...
										for (int i = 0; i < columnPortlets.length; i++) {
											...
											if (stagePortletPreferences.size() > 0) {
												...
												List<PortletPreferences> livePortletPreferences = PortletPreferencesLocalServiceUtil.getPortletPreferences(sourceLiveLayout.getPlid(), portletId);
												if (livePortletPreferences.size() > 0) {
													PortletPreferencesLocalServiceUtil.updatePreferences(0, 3, sourceLiveLayout.getPlid(), portletId, preferences);
												} else {
													Portlet portlet = PortletLocalServiceUtil.getPortletById(portletId);
													PortletPreferencesLocalServiceUtil.addPortletPreferences(
															targetGroup.getCompanyId(), 0, 3, sourceLiveLayout.getPlid(), portletId, portlet, preferences);
												}
											}
										}
									}
								...
								_log.error("Error when publish ext layout!", e);
							}
						}
					}
				}
	...
	private String[] addOrUpdatePortletPreferences(Map<String, String[]> parameterMap, boolean privateLayout){
		String [] rowIds = parameterMap.get("rowIds");
		try {
			if (rowIds != null) {
				for (String plidstr : rowIds) {
					//Get reference extLayout and source layout
					long plid = Long.valueOf(plidstr);
					ExtLayout refStageExtLayout = ExtLayoutLocalServiceUtil.getExtLayout(plid);
					long refSourceGroupId = refStageExtLayout.getSourceGroupId();
					Group sourceGroup = GroupLocalServiceUtil.getGroup(refSourceGroupId);
					String sourceUUID = refStageExtLayout.getSourceUUID();
					Layout sourceStageLayout = LayoutLocalServiceUtil.getLayoutByUuidAndGroupId(sourceUUID, refSourceGroupId, privateLayout);

					//Create portletPreferences in target site
					Layout refStageLayout = LayoutLocalServiceUtil.getLayout(plid);
					UnicodeProperties typeSettingsProperties = refStageLayout.getTypeSettingsProperties();
					String localModulePortletIds = typeSettingsProperties.get("local-module");
					if (StringUtils.isNotEmpty(localModulePortletIds)) {
						String[] columnPortlets = localModulePortletIds.split(StringPool.COMMA);
						for (int i = 0; i < columnPortlets.length; i++) {
							String portletId = columnPortlets[i].trim();
							List<PortletPreferences> stageSourcePortletPreferences = PortletPreferencesLocalServiceUtil.getPortletPreferences(sourceStageLayout.getPlid(), portletId);
							if (stageSourcePortletPreferences.size() > 0) {
								Portlet portlet = PortletLocalServiceUtil.getPortletById(portletId);
								PortletPreferences portletPreferences = stageSourcePortletPreferences.get(0);
								List<PortletPreferences> stageTargetPortletPreferences = PortletPreferencesLocalServiceUtil.getPortletPreferences(plid, portletId);
								if (stageTargetPortletPreferences.size() > 0) {
									PortletPreferencesLocalServiceUtil.updatePreferences(0, 3, plid, portletId, portletPreferences.getPreferences());
								} else {
									PortletPreferencesLocalServiceUtil.addPortletPreferences(
											sourceGroup.getCompanyId(), 0, 3, plid, portletPreferences.getPortletId(), portlet,
			...
			_log.error("Error when add or update portlet refenrence: ", e);
		}
		return rowIds;
	}
