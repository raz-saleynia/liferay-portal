...
public class SimpleCaptchaImpl implements Captcha {
	...
	public void serveImage(
		...
		throws IOException {
	    //get captchaKey from request
	    String captchaAnswerKey = ParamUtil.getString(portletRequest, "captchaAnswerKey");
		...
		CaptchaCacheUtil.put(captchaAnswerKey, simpleCaptcha.getAnswer());
		if (_log.isInfoEnabled()) {
    		_log.info(String.format("Valid Captcha captchaAnswerKey : %s, captchaText : %s", captchaAnswerKey, simpleCaptcha.getAnswer()));
		}
...
//			WebKeys.CAPTCHA_TEXT);
	    ...
	    String captchaAnswerKey = ParamUtil.getString(portletRequest, "captchaAnswerKey");
	    String captchaText = CaptchaCacheUtil.get(captchaAnswerKey);
	    if (_log.isInfoEnabled()) {
            _log.info(String.format("Valid Captcha captchaAnswerKey : %s, captchaText : %s", captchaAnswerKey, captchaText));
        }
		if (captchaText == null) {
			_log.error(
				"Captcha text is null. User " + portletRequest.getRemoteUser() +
					" may be trying to circumvent the captcha. Key:" + captchaAnswerKey);
		...
		CaptchaCacheUtil.remove(captchaAnswerKey);
