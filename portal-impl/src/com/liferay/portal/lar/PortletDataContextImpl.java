...
public class PortletDataContextImpl implements PortletDataContext {
	...
	public void addAssetLinks(Class<?> clazz, long classPK)
		...
		/* ENDPLAY CHANGE TO SORT LINKS */
		Comparator<AssetLink> assetLinkComparator =
				new Comparator<AssetLink>()
				{
					public int compare(AssetLink e1, AssetLink e2) {
						return new Long(e1.getPrimaryKey()).compareTo(e2.getPrimaryKey());
					}
				};
		directAssetLinks = ListUtil.sort(directAssetLinks,assetLinkComparator);
		...
		for (Map.Entry<Integer, List<AssetLink>> entry :
			...
			for (AssetLink assetLink : assetLinks) {
				try {
					...
					assetLinkUuids.add(assetLinkEntry.getClassUuid() + StringPool.STAR + assetLinkEntry.getGroupId());
				}
				catch (NoSuchEntryException nsee) {
				}
			}
