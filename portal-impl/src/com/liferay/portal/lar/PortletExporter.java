...
public class PortletExporter {
	...
	protected void exportAssetCategory(
		...
		//******************************************
		// if it's site replication, add attribute to category
		...
		if (isSiteReplication) {
			boolean isSiteSpecificCategory = SiteReplicationExportUtil.isSiteSpecificCategory(portletDataContext,
					assetCategory.getCategoryId());
			assetCategoryElement.addAttribute(SiteReplicationUtil.ATTR_IS_SITE_SPECIFIC, Boolean.toString(isSiteSpecificCategory));
		}
		//******************************************
		// EXT Override End
		//******************************************
		...
		for (AssetCategoryProperty assetCategoryProperty :
			...
			// EXT Override Start: Off shore
			//******************************************
			// if it's site replication, get static categoryId
			if (isSiteReplication) {
				if (EPCategoryEnum.GLOBAL_CATEGORY_ID.getName().equals(assetCategoryProperty.getKey())) {
					try {
						globalCategoryId = Long.parseLong(assetCategoryProperty.getValue());
		...
		// EXT Override Start: Off shore
		//******************************************
		// if it's site replication, add static categoryId to element
		if (isSiteReplication) {
			List<AssetCategoryProperty> globalAssetCategoryProperties =
					AssetCategoryPropertyLocalServiceUtil.getCategoryProperties(globalCategoryId);
			for (AssetCategoryProperty assetCategoryProperty :
				globalAssetCategoryProperties) {
				if (EPCategoryEnum.STATIC_CATEGORY_ID.getName().equals(assetCategoryProperty.getKey())) {
					assetCategoryElement.addAttribute(SiteReplicationUtil.ATTR_STATIC_CATEGORY_ID, assetCategoryProperty.getValue());
				}
			}
			boolean isGlobalCategory = SiteReplicationExportUtil.isGlobalCategory(assetCategory);
			assetCategoryElement.addAttribute(SiteReplicationUtil.ATTR_IS_GLOBAL_CATEGORY, Boolean.toString(isGlobalCategory));
		}
		//******************************************
		// EXT Override End
		//******************************************
	...
	protected void exportAssetTags(PortletDataContext portletDataContext)
		...
		// if scoped article, then only export tags that are associated with article
		String[] tags = portletDataContext.getParameterMap().get(ExtPortletDataHandlerKeys.SCOPED_TAGS);
		...
		if (scopedArticle) {
			if ((tags != null) && (tags.length > 0)) {
				for (int i = 0; i < tags.length; i++) {
					try {
						AssetTag assetTag = AssetTagLocalServiceUtil.getTag(portletDataContext.getScopeGroupId(), tags[i]);
						if (assetTags == null)
							assetTags = new ArrayList<AssetTag>();
						
						assetTags.add(assetTag);
					} catch (NoSuchTagException nsc) {
						// do nothing
					}
				}
			}
...
//			portletDataContext.getScopeGroupId());
	...
	protected void exportPortlet(
		...
		boolean isSiteReplication = MapUtil.getBoolean(
				portletDataContext.getParameterMap(), ExtPortletDataHandlerKeys.IS_SITE_REPLICATION);
		if (isSiteReplication) {
			boolean isExportingPageTemplate = MapUtil.getBoolean(
					portletDataContext.getParameterMap(), ExtPortletDataHandlerKeys.IS_EXPORTING_PAGE_TEMPLATE);
			if (isExportingPageTemplate) {
				element.addAttribute(SiteReplicationUtil.ATTR_PAGE_TEMPLATE_LAYOUT_PLID,
