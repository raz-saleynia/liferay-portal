...
//			PermissionCacheUtil.clearCache();
			...
			boolean scopedArticle = MapUtil.getBoolean(parameterMap, ExtPortletDataHandlerKeys.SCOPED_ARTICLE);

			if (!scopedArticle) {
				CacheUtil.clearCache();
				// start: disable clearing journal content cache after publishing pages
				Group group = GroupLocalServiceUtil.getGroup(groupId);
				Boolean disableClearingJournalContentCacheAfterPublish =
						ConfigurationPropertyLocalServiceUtil.getBoolean(group.getCompanyId(), groupId,
								"DISABLE.CLEARING.JOURNAL.CONTENT.CACHE.AFTER.PUBLISH");
				if (disableClearingJournalContentCacheAfterPublish == null
						|| !disableClearingJournalContentCacheAfterPublish.equals(Boolean.TRUE)) {
					JournalContentUtil.clearCache(groupId, null, null);
				}
				// end: disable clearing journal content cache after publishing pages
	...
	protected void doImportLayouts(
		...
		boolean isSiteReplication = MapUtil.getBoolean(
				parameterMap, ExtPortletDataHandlerKeys.IS_SITE_REPLICATION);
		// ***************************************
		// EXT Override Start: Off shore
		// ***************************************
		...
		boolean scopedArticle = MapUtil.getBoolean(
				parameterMap, ExtPortletDataHandlerKeys.SCOPED_ARTICLE);
		...
		// EXT Override Start: Off shore
		// ***************************************
		// Typically during publishing of layouts, serviceContext is set during export of layouts.
		// But SiteReplication has separate flows for export and import processes.
		// Therefore the need to explicitly set serviceContext during the import flow.
		...
		// EXT Override Start: Off shore
		// ***************************************
		// if is site replication, update staging group with type settings of source staging group.
		if (isSiteReplication) {
			SiteReplicationImportUtil.importTypeSettingsForGroup(sourceGroupId, groupId);
		}
		// ***************************************
		// EXT Override Start: Off shore
		// ***************************************
		...
		for (Element portletElement : portletElements) {
			...
			// EXT Override Start: Off shore
			//******************************************
			// import portlets for page template layout
			long importedProtoLayoutPlid = 0;
			long plid = 0;
			long previousScopeGroupId = portletDataContext.getScopeGroupId();
			boolean isImportingPortletsInPageTemplate = false;
			if (isSiteReplication) {
				String pageTemplateLayoutPlid = portletElement.attributeValue(
						SiteReplicationUtil.ATTR_PAGE_TEMPLATE_LAYOUT_PLID);
				if (StringUtils.isNotEmpty(pageTemplateLayoutPlid)) {
					long protoLayoutPlid = 0;
					try {
						protoLayoutPlid = Long.parseLong(pageTemplateLayoutPlid);
					} catch (NumberFormatException e) {
						// do nothing
					}
					Map<Long, Layout> importedProtoLayoutMap =
							(Map<Long, Layout>)portletDataContext.getNewPrimaryKeysMap(
									SiteReplicationUtil.KEY_IMPORTED_PROTO_LAYOUTS);
					Layout importedProto = importedProtoLayoutMap.get(protoLayoutPlid);
					if (importedProto == null) {
						_log.error("Can not import portlets for page template layout");
						continue;
					}
					importedProtoLayoutPlid = importedProto.getPlid();
					isImportingPortletsInPageTemplate = true;
				}
			}
			if (importedProtoLayoutPlid != 0) {
				plid = importedProtoLayoutPlid;
			} else {
				//******************************************
				// EXT Override Start: Off shore
				//******************************************
				// skip importing portlets if related layouts not imported.
				Layout newLayout = newLayoutsMap.get(layoutId);
				if (isSiteReplication) {
					if (newLayout != null) {
						plid = newLayout.getPlid();
					} else {
						_log.error("Cannot find imported layout{layout-id:" +
								layoutId + "} for portlet {portlet-id:" +
								portletId + ", so skip importing this portlet!");
						break;
					}
				} else {
			...
			// EXT Override Start: Off shore
			//******************************************
			if (isSiteReplication && isImportingPortletsInPageTemplate) {
				portletDataContext.setScopeGroupId(previousScopeGroupId);
				// add flag for importing portlets of page template
				parameterMap.put(ExtPortletDataHandlerKeys.IS_IMPORTING_PORTLETS_OF_PAGE_TEMPLATE,
	...
	protected void importJournalArticle(
		...
		// EXT Override Start: Off shore
		//******************************************
		// if is site replication, do not import structure/template
		// just using the global group's
		boolean isSiteReplication = MapUtil.getBoolean(
	...
	protected void importLayout(
		...
		// EXT Override Start: Off shore
		//******************************************
		// if is site replication, skip layouts with site specific category
		boolean isSiteReplication = MapUtil.getBoolean(
				portletDataContext.getParameterMap(), ExtPortletDataHandlerKeys.IS_SITE_REPLICATION);
		if (isSiteReplication) {
			Map<Long, Layout> layoutShouldHaveImportedMap =
					(Map<Long, Layout>) portletDataContext.getNewPrimaryKeysMap(
							SiteReplicationUtil.KEY_LAYOUT_SHOULD_HAVE_IMPORTED);
			Map<Long, Layout> layoutSkippedMap =
					(Map<Long, Layout>) portletDataContext.getNewPrimaryKeysMap(
							SiteReplicationUtil.KEY_LAYOUT_SKIPPED);
			layoutShouldHaveImportedMap.put(layout.getPrimaryKey(), layout);
			String extendPath = SiteReplicationImportUtil.getLayoutExtendPath(path);
			String extendXml = portletDataContext.getZipEntryAsString(extendPath);
			if (extendXml != null) {
				Element newRootElement = null;
				try {
					Document document = SAXReaderUtil.read(extendXml);
					newRootElement = document.getRootElement();
					Element isLayoutWithSiteSpecificCategoryElement =
							newRootElement.element(
									SiteReplicationUtil.IS_LAYOUT_WITH_SITE_SPECIFIC_CATEGORY_ELEMENT_NAME);
					boolean isLayoutWithSiteSpecificCategory =
							GetterUtil.getBoolean(isLayoutWithSiteSpecificCategoryElement.attributeValue(
									SiteReplicationUtil.ATTR_VALUE));
					if (isLayoutWithSiteSpecificCategory) {
						layoutSkippedMap.put(layout.getPrimaryKey(), layout);
						return;
					}
				}
				catch (Exception e) {
		...
		// EXT Override Start: Off shore
		//******************************************
		// if is site replication and importing to an existing site.
		// check if the same friendly url layout exists.
		if (isSiteReplication) {
			boolean isImportingIntoExistingSite = MapUtil.getBoolean(
					portletDataContext.getParameterMap(), ExtPortletDataHandlerKeys.IS_IMPORTING_TO_EXISTING_SITE);
			if (isImportingIntoExistingSite) {
		...
		if (existingLayout == null) {
			...
			//EXT OVERRIDE
			importedLayout = LayoutUtil.update(importedLayout, false);
		...
		if (importThemeSettings) {
			...
			// EXT Override Start: Off shore
			//******************************************
			// if is site replication and layout theme is not inherited from layoutset
			// and is not private layout
			if (isSiteReplication && !privateLayout && !layout.isInheritLookAndFeel()) {
				LayoutSet layoutSet = layout.getLayoutSet();
				if (layoutSet != null) {
					if (layout.getThemeId() != null) {
						if (!layout.getThemeId().equals(layoutSet.getThemeId())) {
							// TODO some logging
							Map<Long, Layout> layoutWithSpecificThemeMap =
									(Map<Long, Layout>) portletDataContext.getNewPrimaryKeysMap(
											SiteReplicationUtil.KEY_LAYOUT_WITH_SPECIFIC_THEME);
							layoutWithSpecificThemeMap.put(layout.getPrimaryKey(), layout);
						}
						layout.setThemeId(ThemeImpl.getDefaultRegularThemeId(portletDataContext.getCompanyId()));
						layout.setColorSchemeId(StringPool.BLANK);
					}
				}
			} else {
		...
		// EXT Override Start: Off shore
		//******************************************
		// if is site replication, create page templates
		if (isSiteReplication) {
			String protoLayoutPath = layoutElement.attributeValue(SiteReplicationUtil.ATTR_PROTO_LAYOUT_PATH);
			if (StringUtils.isNotEmpty(layout.getLayoutPrototypeUuid()) && StringUtils.isNotEmpty(protoLayoutPath)) {
				SiteReplicationImportUtil.importLayoutPrototype(groupId, user,
						portletDataContext, importedLayout, protoLayoutPath, layout.getLayoutPrototypeUuid());
			}
		}
		//******************************************
		// EXT Override End: Off shore
		...
		ServiceContext serviceContext = portletDataContext.createServiceContext(
				layoutElement, layout, "layoutsadmin");

		importedLayout.setExpandoBridgeAttributes(serviceContext);

		// have to call this first since there are fields missing in LayoutLocalServiceUtil.updateLayout call
		LayoutUtil.update(importedLayout, false);
		//user local service impl instead so asset entries are created
		serviceContext.setUserId(user.getUserId());

//		// typesettings before running update
//		String typeSettings = importedLayout.getTypeSettingsProperties().toString();

		importedLayout = LayoutLocalServiceUtil.updateLayout(
				groupId, privateLayout, importedLayout.getLayoutId(), parentLayoutId,
				importedLayout.getNameMap(), importedLayout.getTitleMap(),
				importedLayout.getDescriptionMap(), importedLayout.getKeywordsMap(),
				layout.getRobotsMap(), importedLayout.getType(), importedLayout.getHidden(),
				friendlyURL, importedLayout.getIconImage(), iconBytes, serviceContext);

//		UnicodeProperties typeSettingsProperties = importedLayout.getTypeSettingsProperties();
//
//		typeSettingsProperties.fastLoad(typeSettings);
//
//		String finalTypeSettings = typeSettingsProperties.toString();

//		// If the setting have changed
//		if (!typeSettings.equals(finalTypeSettings))
//			// update the typesettings
//			importedLayout = LayoutLocalServiceUtil.updateLayout(
//						groupId,
//						privateLayout,
...
//		importedLayout.setTypeSettings(finalTypeSettings);
//
//		LayoutUtil.update(importedLayout, false);
		...
		// if is site replication, skip layouts with site specific category
		if (isSiteReplication) {
			Map<Long, Long> layoutImportedMap =
					(Map<Long, Long>) portletDataContext.getNewPrimaryKeysMap(
							SiteReplicationUtil.KEY_LAYOUT_IMPORTED);
			layoutImportedMap.put(layout.getPrimaryKey(), importedLayout.getPrimaryKey());
		}
		//******************************************
		// EXT Override End
		//******************************************
