...
import com.endplay.portal.kernel.util.CacheCapacityThreadLocal;
...
public class EhcachePortalCacheManager implements PortalCacheManager {
	...
	protected PortalCache addCache(String name, Cache cache) {
		...
		synchronized (_cacheManager) {
			...
			// EXT to set cache capacity after cache get initialized
			if (_cacheCapacities.get(name) == null) {
				Long cacheCapacity = CacheCapacityThreadLocal.getCacheCapacity();
				if (cacheCapacity != null) {
					setCacheCapacity(name, cacheCapacity.intValue());
					_cacheCapacities.put(name, cacheCapacity);
				}
			}
		}
	...
	private void setCacheCapacity(String cacheName, int capacity) {
		try {
			ObjectName objectName = 
					new ObjectName("net.sf.ehcache:type=CacheConfiguration,CacheManager=liferay-multi-vm-clustered,name="
							+ cacheName);
			_mBeanServer.setAttribute(objectName, new Attribute("MaxElementsInMemory", capacity));
		} catch (Exception ex) {
			_log.error(ex);
		}
	}
	...
	private static Map<String, Long> _cacheCapacities = new HashMap<String, Long>();
