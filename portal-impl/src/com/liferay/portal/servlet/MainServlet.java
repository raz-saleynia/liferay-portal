...
public class MainServlet extends ActionServlet {
	...
	public void service(
		...
		StopWatch s = new StopWatch("MainServlet.service()");
		...
		s.start(getLogTimeString("MainServlet.service().processServicePre()", request));
		...
		try {
			...
			s.start(getLogTimeString("MainServlet.service().callParentService()", request));
		...
		if (_logEPPerf.isInfoEnabled()) {
			_logEPPerf.info(s.shortSummary());
			for (TaskInfo info : s.getTaskInfo()) {
				_logEPPerf.info(info.getTaskName() + " [millis]: " + info.getTimeMillis());
			}
			//_logEPPerf.info(s.prettyPrint());
		}
	...
	protected String getLogTimeString(String msg, HttpServletRequest request) {
		...
		if (request != null) {
			buf.append("host [%s] ");
			buf.append("p_l_id [%d] ");
			buf.append("currentURL [%s] ");
			return String.format(buf.toString(), request.getRemoteHost(), ParamUtil.getLong(request, "p_l_id"), PortalUtil.getCurrentURL(request));
		} else {
	...
	private static final Log _logEPPerf = LogFactoryUtil.getLog("ep.time.log." + MainServlet.class.getName());
