	...
	protected Object getDynamicContent(
		...
		try {
			if (realPath.endsWith(_CSS_EXTENSION) && file.exists()) {
				...
				if ((dynamicContent == null) || (dynamicContent.length() == 0))		// endplay patch
					throw new Exception("Null or zero length file returned");		// endplay patch
			...
			if (!response.isCommitted()) {
				...
				try {
					response.sendError(HttpServletResponse.SC_NOT_FOUND);		// endplay patch
				} catch (Exception e1) {
