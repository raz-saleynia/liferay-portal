...
public class MinifierFilter extends BasePortalFilter {
	...
	public static final int MINIFIER_FILES_LIMIT =
		GetterUtil.getInteger(PropsUtil.get("minifier.files.limit"));
	...
	public void init(FilterConfig filterConfig) {
		...
		if (MINIFIER_FILES_LIMIT > 0) {
			_limitedFilesCache = new LimitedFilesCache<String>(
				MINIFIER_FILES_LIMIT);

			if (_log.isDebugEnabled()) {
				_log.debug(
					"Minifier files limit " + MINIFIER_FILES_LIMIT);
			}
		}
	}
	...
	protected String getSplitMinifiedContent(String minifiedContent, int themeCssSplitCount, int themeCssSplitIndex) {
		int totalSelectorCount = 0;
		int pos = 0;
		int i = 0;
		String splitMinifiedContent = null;
		
		if (themeCssSplitCount <= 0 || themeCssSplitIndex <= 0 || themeCssSplitIndex > themeCssSplitCount) {
			
			return minifiedContent;
		}
		
		while((i = minifiedContent.indexOf("}", pos)) != -1) {
			totalSelectorCount ++;
			pos = ++ i;
		}
		
		int selectorCountPerSplit = totalSelectorCount / themeCssSplitCount;
		int piece = 0;
		int count = 0;
		int start = 0;
		pos = 0;
		...
		while((i = minifiedContent.indexOf("}", pos)) != -1) {
			...
			if (count >= selectorCountPerSplit) {
				piece ++;
				
				if (piece == themeCssSplitIndex) {
					if (themeCssSplitIndex == themeCssSplitCount) {
						splitMinifiedContent = minifiedContent.substring(start);
					} else {
						splitMinifiedContent = minifiedContent.substring(start, pos);
					}
		...
		return splitMinifiedContent;
	}
	
	protected File getSplitMinifiedFile(File file, int themeCssSplitCount, int themeCssSplitIndex) {
		try {
			String minifiedContent = FileUtil.read(file);
			minifiedContent = getSplitMinifiedContent(minifiedContent, themeCssSplitCount, themeCssSplitIndex);
			FileUtil.write(file, minifiedContent);
		} catch (IOException e) {
			_log.error(e);
		}
	...
	protected void processFilter(
		...
		if (minifiedContent == null) {
			...
			int themeCssSplitCount = ParamUtil.getInteger(request, "themeCssSplitCount");
			int themeCssSplitIndex = ParamUtil.getInteger(request, "themeCssSplitIndex");
			
			if (minifiedContent instanceof File) {
				if (themeCssSplitCount > 0 && themeCssSplitIndex > 0) {
					minifiedContent = getSplitMinifiedFile((File)minifiedContent, themeCssSplitCount, themeCssSplitIndex);
				}
				ServletResponseUtil.write(response, (File)minifiedContent);
			}
			else if (minifiedContent instanceof String) {
				if (themeCssSplitCount > 0 && themeCssSplitIndex > 0) {
					minifiedContent = getSplitMinifiedContent((String)minifiedContent, themeCssSplitCount, themeCssSplitIndex);
				}
