...
public class JournalTemplateVelocityResourceListener
	...
	protected InputStream doGetResourceStream(String source) throws Exception {
		...
		try {
			...
			if(_log.isDebugEnabled()) {
				_log.debug("No template found on group " + groupId + " " + templateId);
			}
		}
		
		if(journalTemplate == null) {
			Group globalGroup = GroupLocalServiceUtil.getCompanyGroup(companyId);	
			if(groupId != globalGroup.getGroupId()) {
				journalTemplate =
						JournalTemplateLocalServiceUtil.getTemplate(globalGroup.getGroupId(), templateId);
			}
