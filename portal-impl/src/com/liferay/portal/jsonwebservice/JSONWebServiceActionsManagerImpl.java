...
public class JSONWebServiceActionsManagerImpl
	...
	private int _getParameterPathIndex(String path) {
		...
		// PATCH BEGIN
		// Parameters come in sets, so if we split the string we should get
		// an array of even number of elements
		String parms = path.substring(index+1);
		String[] parts = parms.split(StringPool.SLASH);
		if ((parts.length % 2) != 0)
			return -1;
		// PATCH END
