...
public class XMLStorageAdapter extends BaseStorageAdapter {
	...
	protected long doCreate(
		...
		//ENSM-195: hook ddm content add logic to only add minimal data.
		String[] customizedDDMElements = (String[])serviceContext.getAttribute("customizedDDMElements");
		List<String> customizedDDMElementList = null;
		if (customizedDDMElements != null && customizedDDMElements.length > 0) {
			customizedDDMElementList = Arrays.asList(customizedDDMElements);
		}
		...
		while (itr.hasNext()) {
			...
			if (customizedDDMElementList != null && !customizedDDMElementList.contains(field.getName())) {
				continue;
			}
