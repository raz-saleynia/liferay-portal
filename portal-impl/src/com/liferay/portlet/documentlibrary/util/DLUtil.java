...
import com.liferay.portlet.documentlibrary.util.comparator.*;
...
public class DLUtil {
	...
	public static String getPreviewURL(
		...
		if (DocumentLibraryDataUtil.isRemoteStoreEnabled()) {
			...
			try {
			    previewURL = DocumentLibraryDataUtil.getDocumentSrc(fileEntry.getFileEntryId());
			} catch (Exception e) {
	...
	public static String getThumbnailSrc(
		...
		if (DocumentLibraryDataUtil.isRemoteStoreEnabled()) {
			String defaultThumbnailWidth = null;
			String defaultThumbnailHeight = null;
			
			String defaultThumbnailSize = PropsUtil.get(DocumentLibraryDataConstants.DL_DEFAULT_THUMBNAIL_SIZE);
			StringTokenizer strTokenizer = new StringTokenizer(defaultThumbnailSize,"x");
			while (strTokenizer.hasMoreTokens())
			{
				defaultThumbnailWidth = (String)strTokenizer.nextToken();
				defaultThumbnailHeight = (String)strTokenizer.nextToken();
			}

			thumbnailSrc = DocumentLibraryDataUtil.getThumbnailSrc(fileEntry.getFileEntryId(), new Integer(defaultThumbnailWidth), new Integer(defaultThumbnailHeight));
		}
		else {
			sb.append(themeDisplay.getPathThemeImages());
