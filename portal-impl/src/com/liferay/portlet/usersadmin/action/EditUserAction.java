...
public class EditUserAction extends PortletAction {
	...
	public void processAction(
		...
		try {
			...
			} else if (e instanceof UserLimitsException) {
				SessionErrors.add(actionRequest, e.getClass().getName(), e);

				UserLimitsException userLimitsException = (UserLimitsException) e;
				if (userLimitsException.getType() == UserLimitsException.UPDATE) {
			...
			} else if (e instanceof RoleLimitsException) {
				SessionErrors.add(actionRequest, e.getClass().getName(), e);

				RoleLimitsException roleLimitsException = (RoleLimitsException) e;
				if (roleLimitsException.getType() == RoleLimitsException.UPDATE_STATUS) {
					String redirect = PortalUtil.escapeRedirect(ParamUtil.getString(actionRequest, "redirect"));
