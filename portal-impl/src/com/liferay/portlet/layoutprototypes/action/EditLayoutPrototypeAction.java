...
public class EditLayoutPrototypeAction extends PortletAction {
	...
	protected void updateLayoutPrototype(ActionRequest actionRequest)
		...
		if (layoutPrototypeId <= 0) {
			...
			layoutPrototype = LayoutPrototypeServiceUtil.addLayoutPrototype(
			...
			layoutPrototype = LayoutPrototypeServiceUtil.updateLayoutPrototype(
				layoutPrototypeId, nameMap, description, active);
		}
		
		ExpandoBridge expandoBridge = layoutPrototype.getExpandoBridge();
		long parentGroupId = ParamUtil.getLong(actionRequest, "parentGroup", 0);
		expandoBridge.setAttribute("parentGroupId", new Long(parentGroupId));
